            ...
            snapshot.forEach(doc => {
                ...
                listArea.innerHTML += `
                    <div style="background:white; padding:10px; margin:5px; border-radius:10px; display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <button id="${rozetButonId}" style="background: #f39c12; padding: 5px 10px; border-radius: 5px; border: none; cursor: pointer; color: white;">
                            ⭐ Aferin
                        </button>
                    </div>`;
                ...
                setTimeout(() => {
                    ...
                    if (btn) {
                        btn.addEventListener('click', () => rozetVer(doc.id));
                    }
                }, 0);
            });
        });
